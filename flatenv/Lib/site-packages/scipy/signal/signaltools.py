...
def detrend(data, axis=-1, type='linear', bp=0, overwrite_data=False):
    ...
    overwrite_data : bool, optional
        If True, perform in position detrending and avoid a copy. Default is False
...
def sosfilt(sos, x, axis=-1, zi=None):
    ...
    if dtype.char not in 'fdgFDGO':
        raise NotImplementedError("input type '%s' not supported" % dtype)
    if zi is not None:
        zi = np.array(zi, dtype)  # make a copy so that we can operate in position
    ...
    x = np.array(x, dtype, order='C')  # make a copy, can modify in position
