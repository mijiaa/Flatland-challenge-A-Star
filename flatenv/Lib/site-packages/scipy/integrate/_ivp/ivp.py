...
def prepare_events(events):
    """Standardize event functions and extract is_terminal and incoming_dir."""
    if callable(events):
        events = (events,)
...
def find_active_events(g, g_new, direction):
    ...
    direction : ndarray, shape (n_events,)
        Event "incoming_dir" according to the definition in `solve_ivp`.
...
def solve_ivp(fun, t_span, y0, method='RK45', t_eval=None, dense_output=False,
    ...
    events : callable, or list of callables, optional
        ...
        attributes:
            ...
            incoming_dir: float, optional
                Direction of a zero crossing. If `incoming_dir` is positive,
                `event` will only trigger when going from negative to positive,
                and vice versa if `incoming_dir` is negative. If 0, then either
                incoming_dir will trigger event. Implicitly 0 if not assigned.
    ...
    ``incoming_dir`` fields of an event are applied by monkey patching a function.
