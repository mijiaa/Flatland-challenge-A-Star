...
class TreeObsForRailEnv(ObservationBuilder):
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        #7:
            minimum distance from node to the agent's target given the incoming_dir of the agent if this path is chosen

        #8:
            agent in the same incoming_dir
            n = number of agents present same incoming_dir \
                (possible future use: number of other agents in the same incoming_dir in this branch)
            0 = no agent present same incoming_dir

        #9:
            agent in the opposite incoming_dir
            n = number of agents present other incoming_dir than myself (so conflict) \
                (possible future use: number of other agents in other incoming_dir in this branch, ie. number of conflicts)
            0 = no agent present other incoming_dir than myself
        ...
        #11:
            slowest observed speed of an agent in same incoming_dir
    ...
    def _explore_branch(self, handle, position, direction, tot_dist, depth):
        ...
        # Continue along incoming_dir until next switch or
        # until no transitions are possible along the current incoming_dir (i.e., dead-ends)
        ...
        while exploring:
            ...
            if position in self.location_has_agent:
                ...
                if self.location_has_agent_direction[position] == direction:
                    # Cummulate the number of agents on branch with same incoming_dir
                    ...
                    # If no agent in the same incoming_dir was found all agents in that position are other incoming_dir
            ...
            if num_transitions == 1:
                # Check if dead-end, or if we can go forward along incoming_dir
                ...
                if not last_is_dead_end:
                    # Keep walking through the tree along `incoming_dir`
                    exploring = True
...
class GlobalObsForRailEnv(ObservationBuilder):
    ...
    The observation is composed of the following elements:
        ...
        - obs_agents_state: A 3D array (map_height, map_width, 5) with
            - first channel containing the agents position and incoming_dir
            - second channel containing the other agents positions and incoming_dir
...
class LocalObsForRailEnv(ObservationBuilder):
    ...
    The observation is composed of the following elements:
        ...
        - A 4 elements array with one hot encoding of the incoming_dir.
