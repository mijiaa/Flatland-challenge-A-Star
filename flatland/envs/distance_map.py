...
class DistanceMap:
    ...
    def _distance_map_walker(self, rail: GridTransitionMap, position, target_nr: int):
        ...
        # incoming_dir is the incoming_dir of movement, meaning that at least a possible orientation of an agent
        # in cell (row,col) allows a movement in incoming_dir `incoming_dir'
        nodes_queue = deque(self._get_and_update_neighbors(rail, position, target_nr, 0, enforce_target_direction=-1))

        # BFS from target `position' to all the reachable nodes in the grid
        # Stop the search if the target position is re-visited, in any incoming_dir
        ...
        while nodes_queue:
            ...
            if node_id not in visited:
                ...
                # whose new orientation in the current cell would allow a transition to incoming_dir node[2]
    ...
    def _get_and_update_neighbors(self, rail: GridTransitionMap, position, target_nr, current_distance,
        ...
        if enforce_target_direction >= 0:
            ...
            # This is only possible if the agent has arrived from the cell in the opposite incoming_dir!
