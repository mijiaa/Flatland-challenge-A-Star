...
class RenderLocal(RenderBase):
    ...
    def get_transition_row_col(self, row_col_pos, direction, bgiTrans=False):
        """
        Get the available transitions for row_col_pos in incoming_dir incoming_dir,
        as row & col deltas.

        If bgiTrans is True, return a grid of indices of available transitions.

        eg for a cell row_col_pos = (4,5), in incoming_dir incoming_dir = 0 (N),
    ...
    def plot_single_agent(self, position_row_col, direction, color="r", target=None, static=False, selected=False):
        ...
        direction_row_col = rt.transitions_row_col[direction]  # agent incoming_dir in RC
        direction_xy = np.matmul(direction_row_col, rt.row_col_to_xy)  # agent incoming_dir in xy
