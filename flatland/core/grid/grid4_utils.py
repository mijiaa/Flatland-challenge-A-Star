...
def get_direction(pos1: IntVector2D, pos2: IntVector2D) -> Grid4TransitionsEnum:
    ...
    Returns incoming_dir (int) that can be used with transitions.
    """
    ...
    raise Exception("Could not determine incoming_dir {}->{}".format(pos1, pos2))
...
def direction_to_point(pos1: IntVector2D, pos2: IntVector2D) -> Grid4TransitionsEnum:
    """
    Returns the closest incoming_dir orientation of position 2 relative to position 1
    :param pos1: position we are interested in
    :param pos2: position we want to know it is facing
    :return: incoming_dir NESW as int N:0 E:1 S:2 W:3
    """
